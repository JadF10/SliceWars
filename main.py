...
def main():
    ...
    if not net.connect():
        _show_error(screen, f"Cannot connect to server at {SERVER_HOST}:{SERVER_PORT}")
        pygame.quit()
        sys.exit()
    ...
    while True:
        ...
        # Multiplayer mode
        result = run_game_room(screen, net, username, room_id, players, input_choice)
        if not result:
            continue

        mode, new_input_choice = result

        # Restart input handler if player switched webcam/mouse
        if new_input_choice != input_choice:
            input_choice = new_input_choice
            inp.stop()
            inp = create_input(input_choice)
            if not inp.start():
                print("ERROR: Could not start input device.")
                inp = create_input("mouse")
                inp.start()
                input_choice = "mouse"
        ...
        _play_multiplayer(screen, net, username, mode, room_id, players, inp)
...
def _play_multiplayer(screen, net, username, mode, room_id, players, inp):
    while True:
        game = GameScreen(
            screen        = screen,
            net           = net,
        ...
        if action == "rematch":
            from client.screens.game_room import run as run_game_room_inner
            result = run_game_room_inner(screen, net, username, room_id, players)
            if result:
                new_mode, _ = result
                if new_mode:
                    mode = new_mode
...
def _show_error(screen, message: str):
    ...
    t3 = font_small.render("Make sure server is running, then restart.", True, (180, 180, 180))
