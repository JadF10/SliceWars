...
# Add project root to path so we can import shared/protocol
...
class SliceWarsServer:
    ...
    #  Message router
    # ─────────────────────────────────────────
    def process_message(self, client: Client, raw: str):
        """
        Parse and route every incoming message.
        Stamps timestamp on arrival — Murex signal.
        """
        msg = parse_message(raw)
        if not msg:
            return

        # Stamp arrival time in ms — every message gets a server timestamp
        msg = stamp(msg)
